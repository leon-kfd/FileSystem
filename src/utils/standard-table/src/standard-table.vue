    ...
    fetch () {
      return new Promise((resolve, reject) => {
        ...
        instance.then(data => {
          ...
          if (data) {
            const resultItems = this.getMap(data, this.conf.responseItems ?? this.$STANDARD.responseItems)
            if (typeof resultItems !== 'undefined' && resultItems !== null) {
              if (!this.conf.pagination) {
                ...
                this.conf.data = resultItems
                if (this.conf.pagination) {
                  const resultTotal = this.getMap(data, this.conf.responseTotal ?? this.$STANDARD.responseTotal)
